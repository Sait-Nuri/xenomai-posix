...
int MessageQueueXp::getPrior(){
	...
	if( pthread_getschedparam( pthread_self(), &_policy, &_param) == -1){
		_errno = errno;
		throw ZnmException("Get Priority failed", "getPrior", _errno);
	}
...
int MessageQueueXp::setAttribute(long flag){
	...
	if( mq_setattr(_desc, &_attr, &_prevAttr) == -1){
		_errno = errno;
		throw ZnmException("set attribute failed", "setAttribute()", _errno);
	}
...
int MessageQueueXp::getAttribute(){
	...
	if(mq_getattr( _desc, &_attr) == -1){
		_errno = errno;
		throw ZnmException("Getting attribute failed", "getAttribute()", _errno);
	}
...
int MessageQueueXp::close(){
	...
	if( mq_close(_desc) == -1 ){
		_errno = errno;
		throw ZnmException("close failed", "mq_close()", _errno);
	}
